...
**'BattingPost'** - This table contains individual player batting statistics for each different MLB postseason round from
1903 through 2020. It includes a total of  15,460 unique observations. Each observation contains data that pertains to 
22 different variables including: *'yearID'*, *'round'*, *'playerID'*, *'teamID'*, league ID (*'lgID'*), games played (*'G'*), and 16 
different batting statistics. 

**'SeriesPost'** - This table contains data for series results for each different MLB postseason round from 1903 through 2020.
It includes a total of 358 unique observations. Each observations has data for 9 different variables including: *'yearID'*, 
*'round'*, *'teamIDwinner'*, *'lgIDwinner'*, *'teamIDloser'*, *'lgIDloser'*, *'wins'*, *'losses'*, and *'ties'*. 
...
WS_Batting %>% group_by(yearID, teamID, games_played, Won.Lost) %>% 
  summarize("Total_HR" = sum(HR),
            'mean_Bat_Avg' = mean(Bat_Avg),
            "mean_AB_HR" = mean(AB_HR)) %>%
  ggplot() +
  geom_point(aes(x = yearID, y = Total_HR, color = Won.Lost), position = "dodge") +
  geom_smooth(aes(x = yearID, y = Total_HR, color = Won.Lost)) +
  labs(title = "Total Home Runs Hit in WS") +
  xlab("Year Played") +
  ylab("Total Home Runs Hit") +
  facet_wrap(~games_played, labeller = labeller(games_played =
                                                  c("4" = "4 Game Series",
                                                    "5" = "5 Game Series",
                                                    "6" = "6 Game Series",
                                                    "7" = "7 Game Series")))
...
WS_Batting %>% group_by(yearID, teamID, games_played, Won.Lost) %>% 
  ...
  geom_smooth(aes(x= yearID, y = mean_Bat_Avg, color = Won.Lost)) +
...
**What trend if any exists, in at-bats per home run ratio during the World Series?**
...
WS_Batting %>% group_by(yearID, teamID, games_played, Won.Lost) %>% 
  ...
  geom_smooth(aes(x= yearID, y = mean_AB_HR, color = Won.Lost)) +
...
WS_Batting %>% group_by(yearID, teamID, games_played, Won.Lost) %>% 
  summarize("Total_HR" = sum(HR),
            'mean_Bat_Avg' = mean(Bat_Avg),
            "mean_AB_HR" = mean(AB_HR)) %>%
  ggplot(aes(x = c(Won.Lost), y = Total_HR, fill = Won.Lost)) +
  geom_boxplot(alpha = .3) +
  labs(title = "Total HR Hit") +
  xlab("") +
  ylab("Total Home Runs Hit") +
  facet_wrap(~games_played, labeller = labeller(games_played =
                                                  c("4" = "4 Game Series",
...
## Frequency of At-bats per home run ratio
...
WS_Batting %>% group_by(yearID, teamID, Won.Lost) %>% 
  summarize("Total_HR" = sum(HR),
            'mean_Bat_Avg' = mean(Bat_Avg),
            "mean_AB_HR" = mean(AB_HR)) %>%
  ggplot(aes(x = mean_AB_HR, fill = Won.Lost)) +
  geom_density(alpha = .3) +
  #facet_wrap(~ Won.Lost) +
  labs(title = "At-Bats Per Home Run Ratio") +
  xlab("AB per HR Ratio")
