...
This table already contains a lot of relevant baseball statistics. However it is missing important calculations that are commonly used to determine batting performance.Fortunately, the data needed to perform these calculations is already in the table, so all we have to do is add columns that perform these calculations. More specifically, we will want to look at batting average (AVG), on-base percentage (OBP), slugging (SLG), and on-base plus slugging (OPS).
...
Overall means for batting average (AVG), on-base percentage (OBP), slugging (SLG), and on-base plus slugging (OPS), for each individual postseason series during the expanded wild card era (2012-2019). Let's focus on the expanded wild card era and compare the batting average among each playoff series.

This function takes in our 'PostSeries_Batting' df, groups_by 'Playoff_Series', then summarizes the mean of the chosen stat for each different playoff series, within the chosen start_year and end_year
```{r}
...
We can see that as the playoff series progresses from the wild cards to the division series, championship series, and the world series, there is a drop in all of these statistics. For all four of theses statistics we can see that the wild card series have the highest means. This would be due to the fact that the wild card series is a one game elimination.
 
```{r}
# Plot the results
AVG_Plot <- Series_AVG %>% ggplot(aes(x = reorder(Playoff_Series, -series_mean), y = series_mean)) + geom_point(color = "blue") + ylim(.1, .4) +
  ggtitle("Batting Average Mean for Individual Playoff Series (2012-2019)") +
  xlab("Playoff Series") +
  ylab("Batting Average")


OBP_Plot <- Series_OBP %>% ggplot(aes(x = reorder(Playoff_Series, -series_mean), y = series_mean)) + geom_point(color = "blue") + ylim(.1, .4) +
  ggtitle("On-Base Percentage Mean for Individual Playoff Series (2012-2019)") +
  xlab("Playoff Series") +
  ylab("On-base Percentage")


SLG_Plot<- Series_SLG %>% ggplot(aes(x = reorder(Playoff_Series, -series_mean), y = series_mean)) + geom_point(color = "blue") + ylim(.1, .4) +
  ggtitle("Slugging Percentage Mean for Individual Playoff Series (2012-2019)") +
  xlab("Playoff Series") +
  ylab("Slugging Percentage")


OPS_Plot <- Series_OPS %>% ggplot(aes(x = reorder(Playoff_Series, -series_mean), y = series_mean)) + geom_point(color = "blue") + ylim(.1, .7) +
  ggtitle("On-base Plus Slugging Mean for Individual Playoff Series (2012-2019)") +
  xlab("Playoff Series") +
  ylab("On-base Plus Slugging")

grid.arrange(AVG_Plot, OBP_Plot, SLG_Plot, OPS_Plot)
...
```{r}
# Determine the Winning team for each playoff series during 2012-2019
SeriesWinners_2012_2019 <- SeriesPost %>%
  filter(yearID >= 2012 & yearID <= 2019) %>% 
  rename("Playoff_Series" = round) %>%
  select(yearID, Playoff_Series, teamIDwinner)


# Filter our 'PostSeries_Batting' data for years 2012-2019
Bat_2012_2019 <- PostSeries_Batting %>% 
  filter(yearID >= 2012 & yearID <=2019) %>%
  select(yearID, Playoff_Series, teamID, lgID, AVG, OBP, SLG, OPS) %>%
  group_by(yearID, Playoff_Series, teamID) %>%
...
SeriesWin_Stats <- inner_join(SeriesWinners_2012_2019, Bat_2012_2019, by = c("yearID", "Playoff_Series", "teamIDwinner" = "teamID"))
...
SeriesWin_Stats %>% ggplot(aes(x = AVG, fill = Playoff_Series)) + geom_density(alpha = .3) + facet_wrap(~Playoff_Series)
